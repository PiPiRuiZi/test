...
def run_htmlTest():
    ...
    runner = HTMLTestRunner.HTMLTestRunner(
        ...
        verbosity=1,
        stream=open(file="QQ登陆测试报告.html", mode="w+", encoding="utf-8")
    )
    runner.run(tests)
...
def send_email():
    ...
    # 读出文件
    with open("QQ登陆测试报告.html", encoding="utf-8") as file:
        content = file.read()
    ...
    part.add_header('Content-Disposition', 'attachment', filename='QQ登录测试报告.html')
