...
def bank_add_user(username, password, money, country, province, street, door):
    # 创建连接
    con = pymysql.connect(host="localhost", user="root", passwd="111111", db="bank")
    # 创建控制台
    cur = con.cursor()
...
def add_user():
    ...
    if money.isdigit():
        ...
        if status is False:
            ...
            con = pymysql.connect(host="localhost", user="root", passwd="111111", db="bank")
            cur = con.cursor()
            sql = "select * from user where id = {0}".format(account)
            cur.execute(sql)
            print("开户成功！")
            info = """
                ----------个人信息【建设银行】---------
...
def bank_add_money(account, add_money):
    ...
    for i in range(length):
        if account == data[i][0]:
            ...
            break
    if tmp:
        sql = "update user set money = money + {0} where id = {1}".format(add_money, account)
        cur.execute(sql)
        # 提交数据到数据库
        con.commit()
        return True
            ...
            while True:
                ...
                if money.isdigit():
                    ...
                    if status:
                        con = pymysql.connect(host="localhost", user="root", passwd="111111", db="bank")
                        cur = con.cursor()
                        sql = "select money from user where id = {0}".format(account)
                        cur.execute(sql)
                        data = cur.fetchone()
                        con.commit()
                        con.close()
...
def bank_take_money(account1, password1, money1):
    con = pymysql.connect(host="localhost", user="root", passwd="111111", db="bank")
    cur = con.cursor()
    sql = "select id,password,money from user where id = {0}".format(account1)
    cur.execute(sql)
    data = cur.fetchone()
    if data != None and account1 == data[0]:
        if password1 == data[1]:
            if money1 > data[2]:
                ...
                sql = "update user set money = money - {0} where id = {1}".format(money1, account1)
                cur.execute(sql)
                # 提交数据到数据库
                con.commit()
                con.close()
            ...
            while True:
                ...
                if money1.isdigit():
                    ...
                    if data == 3:
                        ...
                        con = pymysql.connect(host="localhost", user="root", passwd="111111", db="bank")
                        cur = con.cursor()
                        sql = "select money from user where id = {0}".format(account1)
                        cur.execute(sql)
                        data = cur.fetchone()
                        con.commit()
                        con.close()
...
def transfer():
    ...
    while True:
        ...
        if account.isdigit():
            account = int(account)
            sql = "select id, password, money from user where id = {0}".format(account)
            cur.execute(sql)
            con.commit()
            data = cur.fetchone()
            if account == data[0]:
                        ...
                        while True:
                            ...
                            if payee.isdigit():
                                ...
                                if payee == account:  # 收款人不能是自己
                                    ...
                                    sql = "select id, password, money from user where id = {0}".format(payee)
                                    cur.execute(sql)
                                    con.commit()
                                    data1 = cur.fetchone()
                                        ...
                                        while True:
                                            ...
                                            if s.isdigit():
                                                ...
                                                if s > data[2]:  # 判断余额是否充足
                                                    ...
                                                    sql = "update user set money = money - {0} where id = {1}".format(s, account)
                                                    cur.execute(sql)
                                                    sql = "select money from user where id = {0}".format(account)
                                                    cur.execute(sql)
                                                    con.commit()
                                                    ...
                                                    sql = "update user set money = money + {0} where id = {1}".format(s, payee)
                                                    cur.execute(sql)
                                                    con.commit()
                                                    con.close()
                                                    cur.close()
