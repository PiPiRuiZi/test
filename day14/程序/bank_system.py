...
def bank_add_user(username, password, money, country, province, street, door):
    ...
    param = [account, username, password, money, country, province, street, door, __bank_name]
    update(sql, param)
    return account
    ...
    if money.isdigit():
        ...
        if status == 0:
            ...
            print(info.format(account=account, username=username, password=password,
                              ...
                              street=street, door=door, bank_name_tmp=__bank_name))
    else:
        print("输入不合法！请重新输入")
...
def bank_take_money(account1, password1, money1):
    ...
    param = [account1]
    data = select(sql, param, "one")
    if data is not None and account1 == data[0]:
        if password1 == data[1]:
            if money1 > data[2]:
                return 2  # 余额不够
            else:
...
# 转账
def bank_transfer(account, payee, s):
    try:
        sql = "update user set money = money - %s where id = %s"
        param = [s, account]
        update(sql, param)
                        ...
                        while True:
                            ...
                            if payee.isdigit():
                                ...
                                if payee == account:  # 收款人不能是自己
                                    ...
                                    if data1 is not None and payee == data1[0]:  # 判断收款人是否存在
                                        while True:
                                            s = input("请输入转账金额：")
                                            if s.isdigit():
                                                s = int(s)
...
def inquiry():
    while True:
        ...
        if account.isdigit():
            ...
            param = [account]
            data = select(sql, param, "one")
            if data is not None and account == data[0]:
                while True:
